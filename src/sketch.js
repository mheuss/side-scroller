/*
       WARNING:
        ...
        You'd be better off looking at the zip or the repo, and reading the README.md file
...
/**
 * Quick and easy names for the various colors
 * @type {{foliageOutline: number[], lightGray: number[], darkFoliage: number[], lightFoliage: number[], cloudBorder: number[], darkerGray: number[], coinHighlight: number[], stoneGray: number[], blueSky: number[], cloudBlush: number[], grassGreen: number[], darkBark: number[], coinColor: number[], coinOutline: number[], lightBark: number[], midGray: number[]}}
 */
export const colors = {
...
/**
 * This is the main function for the game, and contains the p5 instance
 * and renders the game, initializes all values, calls all the models,
 * sets up all the callbacks.
 *
 * @param p5 Instance of p5
 */
function sketch(p5) {
  ...
  p5.draw = () => {
    ...
    // behind him
    orderedRenders.forEach((model) => {
      model.draw();
    });
  };
}
/**
 * Wraps the sketch in a React Fragment
 * @constructor
 */
...
/*
  The below documentation is autogenerated from the class below.
...
export class Collectable extends Sprite {
  ...
  transformSmile = () => {
    ...
    if (this.p5.frameCount % 5 === 0) {
      // Change the smile by the amount we want to change it
      this.smileArc += this.transformSmileIncrement;
      // If we get out of bounds, let's kill the transformation increment.
...
export class PieceOfPaper extends Sprite {
   ...
   *
   * This takes more memory, it's true. But it removes the conditionals
